...
/* ── the lens — a slow, randomly-wandering magnifier over the
   graph-paper grid, shared with the home screen. Position eases
   toward a fresh random target on an irregular interval, and its
   scale breathes on a slow sine, so the motion reads as ambient
   drift rather than a scripted loop. ── */
(function(){
  const el=document.getElementById('lens'); if(!el) return;
  if(matchMedia('(prefers-reduced-motion: reduce)').matches) return;

  let x=innerWidth*0.5, y=innerHeight*0.45;
  let tx=x, ty=y, retimer=null;

  function pickTarget(){
    const m=60;
    tx=m+Math.random()*Math.max(1,innerWidth-m*2);
    ty=m+Math.random()*Math.max(1,innerHeight-m*2);
    clearTimeout(retimer);
    retimer=setTimeout(pickTarget,7000+Math.random()*7000);
  }
  pickTarget();

  const t0=performance.now();
  function frame(t){
    x+=(tx-x)*0.0045; y+=(ty-y)*0.0045;
    const breathe=1+Math.sin((t-t0)/8600)*0.14;
    el.style.left=x+'px'; el.style.top=y+'px';
    el.style.transform='translate(-50%,-50%) scale('+breathe.toFixed(3)+')';
