...
(function(){
  ...
  // ellipsoid bounds for the joint-gap dot cluster: a lens/sphere-like
  // shape that tucks inward toward the poles as it nears the outer
  // radius, rather than a flat-topped disc with a hard, exposed edge —
  // reads as sitting inside the bone form instead of poking out of it.
  const JOINT_R0=0.34, JOINT_Y0=0.13;
  function buildJointDots(){
    jointDots=[];
    const N = innerWidth<760 ? 9000 : 16000; // 50x further increase, per feedback
    for(let i=0;i<N;i++){
      const ang=Math.random()*6.28318, rad=0.05+Math.random()*(JOINT_R0-0.05);
      const maxY=JOINT_Y0*Math.sqrt(Math.max(0,1-(rad/JOINT_R0)*(rad/JOINT_R0)));
      jointDots.push({
        x:Math.cos(ang)*rad, y:(Math.random()-.5)*2*maxY, z:Math.sin(ang)*rad,
  ...
  function drawJointDots(c,AMB,MS){
    ...
    // additive blending: at this density, individual motes overlap
    // and their brightness accumulates naturally into a soft glow
    // at the densest point — cheaper than a per-dot gradient halo
    // and reads as a genuine bloom rather than flat stacked circles
    ...
    for(const d of jointDots){
      ...
      ctx.fillStyle='rgba('+rC+','+gC+','+bC+','+coreA.toFixed(3)+')'; ctx.fill();
    }
  ...
  function draw(){
    ...
    /* ambient evidence point-cloud dots removed per feedback — they
       read as stray gray dots scattered across the page background. */
    ...
    /* ambient drift dots removed per feedback — was a scatter of
       twinkling dots across the whole page background. */
  }
