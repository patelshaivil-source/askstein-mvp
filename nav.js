...
/* ── page-exit fade ("fade away") ─────────────────────────────
   Clicking any internal .html link fades the current page out
   before the browser navigates, so leaving a page is the mirror
   of arriving on one instead of a hard cut to blank white. Skips
   modified clicks (new-tab, new-window), external links, links
   targeting a different frame, and same-page hash links — those
   should behave exactly like normal links. */
if(!window.matchMedia('(prefers-reduced-motion: reduce)').matches){
  document.addEventListener('click', function(e){
    ...
    e.preventDefault();
    const pg = document.querySelector('.page-shift');
    if(!pg){ location.href = a.href; return; }
    pg.classList.remove('in');
    pg.classList.add('leaving');
    /* matches --dur-fast (0.4s) in style.css — keep these in sync */
